#Univariate version of maxdist(), a function of the convevol package (Stayton, 2015).
#v0.2
...
maxdist1d<-function (phyl, phendata, t1, t2) 
{
    require(convevol)
    ...
    combineddata <- cbind(anctimes, alldata) #Might need to look at this again
    mrcas <- mrca(phyl)
    ...
    alldist<-c()
    for (i in 1:length(t1pathtraits)) {
        for (j in 1:length(t2pathtraits)) {
            alldist<-c(alldist,abs(t1pathtraits[i]-t2pathtraits[j]))
            }
        }
    answer <- max(alldist)
}
